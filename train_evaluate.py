...
from model_evaluator import evaluate_after_training, EvaluationConfig, ModelEvaluator
...
class FinetuningConfig:
    ...
    eval_test_dataset: Any = None
    eval_output_dir: Optional[str] = None
    eval_save_plots: bool = True
    eval_save_predictions: bool = True
    eval_detailed_analysis: bool = True
    eval_include_per_sample_analysis: bool = False
    eval_plot_format: str = "png"
    eval_show_plots: bool = False
...
def run_comprehensive_evaluation(
    ...
    """
    Run comprehensive evaluation using the ModelEvaluator
    
    Args:
        model_path: Path to the saved model
        test_dataset: Test dataset for evaluation
        config: Training configuration with evaluation parameters
    ...
    eval_output_dir = config.eval_output_dir or os.path.join(model_path, "comprehensive_evaluation")
    ...
    eval_config = EvaluationConfig(
        ...
        save_predictions=config.eval_save_predictions,
        save_plots=config.eval_save_plots,
        save_metrics=True,
        plot_format=config.eval_plot_format,
        dpi=300,
        figsize=(12, 8),
        use_wandb=config.use_wandb,
        wandb_project=f"{config.wandb_project}-evaluation",
        wandb_run_name=f"{config.model_name}_evaluation",
        show_plots=config.eval_show_plots,
        detailed_analysis=config.eval_detailed_analysis,
        include_per_sample_analysis=config.eval_include_per_sample_analysis
    )
...
def finetune_audio_model(
    ...
    try:
        ...
        # Run comprehensive evaluation on test dataset if provided
        if config.run_evaluation and config.eval_test_dataset is not None:
            print("\n" + "="*60)
            ...
            try:
                eval_results = run_comprehensive_evaluation(
                    ...
                    test_dataset=config.eval_test_dataset,
                    config=config
                )
                
                # Add evaluation results to training results
                training_results.update({
                    'comprehensive_evaluation': eval_results
                })
                
                # Log evaluation results to W&B if available
                if config.use_wandb and wandb.run and eval_results:
                    wandb.log({
                        "comprehensive_eval_accuracy": eval_results.get('overall_metrics', {}).get('accuracy', 0),
                        "comprehensive_eval_f1": eval_results.get('overall_metrics', {}).get('f1_weighted', 0),
                        "comprehensive_eval_precision": eval_results.get('overall_metrics', {}).get('precision_weighted', 0),
                        "comprehensive_eval_recall": eval_results.get('overall_metrics', {}).get('recall_weighted', 0),
                    })
                    
            except Exception as eval_error:
                print(f"⚠️ Comprehensive evaluation failed: {eval_error}")
                training_results['comprehensive_evaluation_error'] = str(eval_error)
        ...
        print("="*60)
        print(f"🎯 Final Accuracy: {final_metrics.get('eval_accuracy', 'N/A'):.4f}")
        print(f"📈 Final F1 Score: Training evaluation metrics available in logs")
        print(f"⏱️ Training Runtime: {train_result.metrics['train_runtime']:.2f}s")
        print(f"💾 Model saved to: {config.model_name}")
        
        if config.run_evaluation and 'comprehensive_evaluation' in training_results:
            comp_eval = training_results['comprehensive_evaluation']
            if comp_eval and 'overall_metrics' in comp_eval:
                print(f"🔬 Comprehensive Test Accuracy: {comp_eval['overall_metrics']['accuracy']:.4f}")
                print(f"📊 Comprehensive Test F1: {comp_eval['overall_metrics']['f1_weighted']:.4f}")
...
try:
    ...
    print("⚠️ dataset_preprocessor not found. Please ensure it's available for the example to work.")
    dataset_whisper, fe_whisper, l2i_w, i2l_w, info_w = None, None, None, None, None


def example_usage():
    """Example of how to use the finetuning function with evaluation"""
    
    if dataset_whisper is None:
        print("⚠️ Dataset preprocessing failed. Cannot run example.")
        return None, None, {}
    
    # Configure finetuning with evaluation
    config = FinetuningConfig(
        ...
        eval_test_dataset=dataset_whisper.get('test'),  # Use test split if available
        eval_save_plots=True,
        eval_save_predictions=True,
        eval_detailed_analysis=True,
        eval_plot_format="png",
        eval_show_plots=False
    )
    ...
    if 'comprehensive_evaluation' in results:
        ...
        if comp_eval and 'overall_metrics' in comp_eval:
            ...
            print(f"Test accuracy: {comp_eval['overall_metrics']['accuracy']:.4f}")
            print(f"Test F1 score: {comp_eval['overall_metrics']['f1_weighted']:.4f}")
